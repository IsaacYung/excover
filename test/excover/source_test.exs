defmodule ExCover.SourceTest do
	use ExUnit.Case
  require Logger
	...
	# test "compile info" do
		#	info = ExCover.Source.get_compile_info(@mod)
	...
	test "source info" do
		source = ExCover.Source.get_source_path(@mod)

		assert is_list(source)
	end

	test "get source and quoted" do
		{quoted, source} = ExCover.Source.get_quoted_source(@mod)
	...
	test "check 1 alias" do
		as = ExCover.Source.alias_mod(X)
		assert [:X] = as
    m = ExCover.Source.alias_to_atom(as ++ [:Elixir])
    assert X == m
	end

	test "check 2 aliases" do
		as = ExCover.Source.alias_mod(@mod)
		assert [:ExCover, :Source] == as
    m = ExCover.Source.alias_to_atom([:Elixir | as] |> Enum.reverse)
    assert @mod == m
	end
    ...
    mods = ExCover.Source.find_all_mods_and_funs(quoted)
    # IO.inspect mods
    assert mods[X][X] == 1
	end
  ...
  test "find all modules and funs" do
    ...
    all_mods = ExCover.Source.find_all_mods_and_funs(mod)
  ...
  test "generate line entries" do
    ...
    lines = ExCover.Source.generate_lines(cover, m)

    f_link = ExCover.Task.module_anchor({X, :f, 2})
  ...
  test "find Protocols and implementations" do
    ...
    all_mods = ExCover.Source.find_all_mods_and_funs(mods)
  ...
  test "Merge coverage data of a single file" do
    ...
    merged = ExCover.Source.merge_coverage(mod)
